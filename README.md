...
FiraCode 与 Maple Mono 的高品质融合等宽字体（Surgical Selective Hinting Edition）。
...
* **精确居中与 2:1 等宽**：
  * 中文字形 advance 严格设定为 2 倍拉丁格宽，杜绝终端错位与漂移。
  * 基于轮廓真实 `xmin` 动态计算 `hmtx.lsb`，彻底修复贴边与字符变窄 Bug，字符自然居中饱满。
* **外科手术式 Hinting 注入**：
...
├── merged-v4/          # 构建产物目录 (*.ttf)
├── scripts/
│   ├── build.py        # 完整端到端自动化构建管线
